...
and difficulties commensurating state of evidence to form a coherent
understanding of the phenomon of label categorization. A recent
...
(2010); Ware \& Booth (2010); Booth \& Waxman (2006) ; Susan S. Jones \&
...
\begin{CodeChunk}
\begin{figure}[tb]
\includegraphics[width=1\linewidth]{conceptual_diagram} \caption[A diagram to visualize different factor that potentially contribute to the origin of the shape bias ]{A diagram to visualize different factor that potentially contribute to the origin of the shape bias }\label{fig:prisma }
\end{figure}
\end{CodeChunk}
...
graham and ). In additon, many children explored whether their chosen
test object could perform the intended function after selecting it based
on shape, specially in case of designs by which the visual inspection of
the affordance was not certain (the dax for example. This behavior
...
shape might also share functionality. In the case of the ``Zimbo'', the
object was designed in away that makes the function afforded only by a
part, but the whole structure is irrelavant. Hence, we see children go
with function in the function condition, however, the objects shared the
shape of a small part that is critical for performing the function. This
complicates the interpretation of the results as the shape of the part
but not the whole is shared across chosen objects. For evaulating the
children's ability to reason about intrinsic affordances, we created the
``Fep'' in a way such that the material itself is very critical for
performing the function ``holding water while being made of paper
towel''.
