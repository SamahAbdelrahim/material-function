...
Twenty four typically developing English speaking participants (2-5 years old, mean=`r round(mean(e1_subs$age_mo)/12,2)`, SD=`r round(sd(e1_subs$age_mo/12),2)`) were recruited from a local nursery school and children’s museum in the US (preregistration link).
...
31 (target n=96, 24 per each age group) participants between 2-5 years old (mean=`r round(mean(e2_subs$age_mo),2)`, SD=`r round(sd(e2_subs$age_mo),2)`, n per age group) were recruited from a local nursery school in the US.
